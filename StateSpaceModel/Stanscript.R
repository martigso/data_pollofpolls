...
pp <- read.csv("data_pollofpolls/data/polls_long.csv", fileEncoding = "UTF-8", stringsAsFactors = FALSE)
...
##### STAN    #####
oslo_model <- stan("state_space_polls_hg.stan", iter=200,
                   chains=1,
...
musum <- data.frame(summary(oslo_model, pars = munames)$summary)
...
electiondayiterations <- extract(oslo_model, pars=munames)
...
save(oslo_model, file="stanresult_200iterations.rdata")



######        Plot          #####
plot(mu_oslo_ap$date, mu_oslo_ap$median,
     bty="n", type="n", axes=FALSE,
     ylab="", xlab="",
     ylim=c(0, 35))
abline(v=as.numeric(c(seq(as.Date("2017/04/01"), by = "month", length.out = 6), as.Date("2017-09-11"))),
       lty="dashed",col=grey(0.4, alpha=.5))
axis(1, at=as.numeric(c(seq(as.Date("2017/04/01"), by = "month", length.out = 6), as.Date("2017-09-11"))),
     labels=c(as.character(seq(as.Date("2017/04/01"), by = "month", length.out = 6)), "Election day"),
     las=2,
     cex.axis=0.7, col="white")
axis(2, at=seq(0, 100, 5), las=2, col="white")
abline(h=seq(0, 100, 2.5),lty="dashed", col=grey(0.4, alpha=.5))
for(i in 1:length(munames)){
  polygon(c(mu_oslo_ap$date, rev(mu_oslo_ap$date)), 
          c(musum[grep(munames[i], musum$id),"X2.5."],
            rev(musum[grep(munames[i], musum$id),"X97.5."])),
          border=FALSE,
          col=colvector[i])
}
...
abline(h=seq(0, 100, 2.5),lty="dashed", col=grey(0.4, alpha=.5))
polygon(c(mu_oslo_ap$date, rev(mu_oslo_ap$date)), 
        c(mu_oslo_ap$lower, rev(mu_oslo_ap$upper)),
        border=FALSE, col="#94001870")
polygon(c(mu_oslo_h$date,  rev(mu_oslo_h$date)), 
        c(mu_oslo_h$lower, rev(mu_oslo_h$upper)),
        border=FALSE, col="#08029470")
lines(mu_oslo_ap$date, mu_oslo_ap$median,lwd=2, col="#94001870")
lines(mu_oslo_h$date, mu_oslo_h$median  ,lwd=2, col="#080294")
points(polls_oslo_ap$date, polls_oslo_ap$prosent, pch=16, col="#94001870")
points(polls_oslo_h$date, polls_oslo_h$prosent, pch=16, col="#080294")
